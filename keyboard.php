...
if ($PaySettingnow == "onnowpayment") {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['textnowpayment'], 'callback_data' => "plisio", 'style' => 'primary']
    ];
}
if ($payment_status_nowpayment == "1") {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['textsnowpayment'], 'callback_data' => "nowpayment", 'style' => 'primary']
    ];
}
...
if ($Swapino == "onSwapinoBot") {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['iranpay2'], 'callback_data' => "iranpay1", 'style' => 'primary']
    ];
}
if ($trnadoo == "onternado") {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['iranpay3'], 'callback_data' => "iranpay2", 'style' => 'primary']
    ];
}
if ($arzireyali3 == "oniranpay3" && $paymentexits >= 2) {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['iranpay1'], 'callback_data' => "iranpay3", 'style' => 'primary']
    ];
}
if ($PaySettingaqayepardakht == "onaqayepardakht") {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['aqayepardakht'], 'callback_data' => "aqayepardakht", 'style' => 'primary']
    ];
}
if ($zarinpal == "onzarinpal") {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['zarinpal'], 'callback_data' => "zarinpal", 'style' => 'primary']
    ];
}
if ($paymentstatussnotverify == "onverifypay") {
    $step_payment['inline_keyboard'][] = [
        ['text' => $datatextbot['textpaymentnotverify'], 'callback_data' => "paymentnotverify", 'style' => 'primary']
    ];
}
...
$list_marzban_panel_users_change['inline_keyboard'][] = [
    ['text' => $textbotlang['users']['backbtn'], 'callback_data' => "backorder", 'style' => 'primary'],
];
...
while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
    if ($result['hide_user'] != null and in_array($from_id, json_decode($result['hide_user'], true)))
        continue;
    $list_marzban_panel_usertest['inline_keyboard'][] = [
        ['text' => $result['name_panel'], 'callback_data' => "locationtest_{$result['code_panel']}", 'style' => 'primary']
    ];
}
$list_marzban_panel_usertest['inline_keyboard'][] = [
    ['text' => $textbotlang['users']['backbtn'], 'callback_data' => "backuser", 'style' => 'danger', 'icon_custom_emoji_id' => 5258236805890710909],
...
$optionX_ui_tunnel = json_encode([
    'keyboard' => [
        [['text' => "⚙️ وضعیت قابلیت ها پنل"]],
        [['text' => "✍️ نام پنل"], ['text' => "❌ حذف پنل"]],
        [['text' => "🌐 تنظیم آی‌پی/دامنه سرور"], ['text' => "🔌 مدیریت پورت‌های تانل"]],
        [['text' => "🔐 ویرایش رمز عبور"], ['text' => "👤 ویرایش نام کاربری"]],
...
while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $list_departman['inline_keyboard'][] = [
        ['text' => $result['name_departman'], 'callback_data' => "departman_{$result['id']}", 'style' => 'primary', 'icon_custom_emoji_id' => 5429398777419014515]
    ];
}
$list_departman['inline_keyboard'][] = [
    ['text' => $textbotlang['users']['backbtn'], 'callback_data' => "backuser", 'style' => 'danger', 'icon_custom_emoji_id' => 5258236805890710909],
...
function KeyboardProduct($location, $query, $pricediscount, $datakeyboard, $statuscustom = false, $backuser = "backuser", $valuetow = null, $customvolume = "customsellvolume")
{
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $btn = [
            'text' => $result['name_product'],
            'callback_data' => "{$datakeyboard}{$result['code_product']}{$valuetow}",
        ];

        if (!empty($result['panel_color'])) {
            $btn['style'] = $result['panel_color'];
        }

        if (!empty($result['panel_emoji'])) {
            if (preg_match('/emoji-id="(\d+)"/', $result['panel_emoji'], $matches)) {
                $btn['icon_custom_emoji_id'] = (int) $matches[1];
            } elseif (is_numeric($result['panel_emoji'])) {
                $btn['icon_custom_emoji_id'] = (int) $result['panel_emoji'];
            }
        }
...
function KeyboardCategory($location, $agent, $backuser = "backuser")
{
    ...
    $list_category['inline_keyboard'][] = [
        ['text' => "▶️ بازگشت به منوی قبل", "callback_data" => $backuser, 'style' => 'danger'],
    ];
