...
// list departeman
 ...
 $stmt->execute();
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $list_departman['inline_keyboard'][] = [['text' => $result['name_departman'], 'callback_data' => "departman_{$result['id']}" , 'style' => 'primary' , 'icon_custom_emoji_id'=> 5429398777419014515]
            ];
    }
$list_departman['inline_keyboard'][] = [
    ['text' => $textbotlang['users']['backbtn'], 'callback_data' => "backuser", 'style'=>'danger' , 'icon_custom_emoji_id'=> 5258236805890710909],
...
function KeyboardProduct($location,$query,$pricediscount,$datakeyboard,$statuscustom = false,$backuser = "backuser", $valuetow = null,$customvolume = "customsellvolume"){
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $product['inline_keyboard'][] = [
                ['text' =>  $result['name_product'], 'callback_data' => "{$datakeyboard}{$result['code_product']}{$valuetow}", 'style'=>'success' , 'icon_custom_emoji_id'=> 5958508069334813629]
            ];
    }
